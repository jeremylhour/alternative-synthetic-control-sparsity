...
Simu <- function(Rep=1000,N,P,R2y=0.8,R2d=0.2,c1=0.7,c2=2){
  
  ## STEP A. SIMULATIONS
  
  R <- Rep
  Results <- matrix(ncol=5, nrow=R)
  Tstat <- matrix(ncol=5, nrow=R)
  ...
  for(r in 1:R){
    ...
    Tstat[r,] <- c(ImmunizedATT(y,d,X,CAL$betaLasso, Immunity=F)$tstat,
                   #ImmunizedATT(y,d,X,CAL$betaPL, Immunity=F)$tstat,
                   ImmunizedATT(y,d,X,CAL$betaLasso,ORT_WLS_L$muLasso, Immunity=T)$tstat,
                   #ImmunizedATT(y,d,X,CAL$betaPL,ORT_WLS_PL$muPL, Immunity=T)$tstat,
                   ImmunizedATT(y,d,X,LOGIT$betaLasso, Immunity=F)$tstat,
                   #ImmunizedATT(y,d,X,LOGIT$betaPL, Immunity=F)$tstat,
                   #BCH$tstat,
                   ImmunizedATT(y,d,X,LOGIT$betaLasso, FARRELL$muLasso, Immunity=T)$tstat,
                   ImmunizedATT(y,d,X,LOGIT$betaPL, FARRELL$muPL, Immunity=T)$tstat)
  ...
  return(list(Results, AsySD, Tstat, Convergence, StatDisplay))
...
N50P50 <- Simu(N=50,P=50)
N50P100 <- Simu(N=50,P=100)

N100P50 <- Simu(N=100,P=50)
N100P100 <- Simu(N=100,P=100)
N100P200 <- Simu(N=100,P=200)

N200P50 <- Simu(N=200,P=50)
N200P100 <- Simu(N=200,P=100)
N200P200 <- Simu(N=200,P=200)
N200P500 <- Simu(N=200,P=500)

N500P50 <- Simu(N=500,P=50)
N500P100 <- Simu(N=500,P=100)
N500P200 <- Simu(N=500,P=200)
N500P500 <- Simu(N=500,P=500)
...
write.table(res, file = "table_simu_base.txt", append = FALSE, quote = FALSE, sep = " & ",
            eol = paste(" \\\\ \n"), na = "--", dec = ".", row.names = T,
            col.names = T)
