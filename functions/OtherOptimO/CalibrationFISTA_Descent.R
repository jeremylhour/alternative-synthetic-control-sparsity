CalibrationFISTA_Descent <- function(betaInit=rep(0,ncol(X)),d,X,lambda,
                             tol=1e-6,maxIter=1000,maxLineSearch=10,trace=F){
  ### Computes Lasso solution for Calibration objective function
  ### with FISTA Descent method with backtracking line search
  ### Jeremy L Hour
  ### 14 janvier 2016
  ...
  repeat{
    ...
    repeat{
      j <- j+1
      u <- prox(z - t * gammagrad(z,d,X), lambda*t)
      if(LassoObj(u,d,X,lambda) < LassoObj(beta,d,X,lambda)) beta <- u
      if(gamma(beta,d,X) < gamma(z,d,X) + t(gammagrad(z,d,X))%*%(beta-z) + sum((beta-z)^2)/(2*t) || j > maxLineSearch) break
      t <- b*t
    }
    
    v <- betaO + (u - betaO)/theta
